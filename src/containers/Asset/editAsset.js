...
import { editAsset_Permission} from '../shared/permissionMenu';
...
export default function EditForm(props) {
    reloadToHomeNotAuthorize(editAsset_Permission,'TRANSACTION');
    ...
    const successHandler = (data) =>{
        if(data.data){
            let det = data.data;
            setInputKodeAsset(det.kodeasset);
            setSelJenisAsset(det.assettype);
            setInputKepalaNama(det.kepala_nama);
            setInputKepalaNoPolisi(det.kepala_nopolisi);
            setSelKepalaJenisKendaraan(det.kepala_jeniskendaraan);
            setInputKepalaMerk(det.kepala_merk);
            setInputKepalaNoMesin(det.kepala_nomesin);

            setInputKepalaNoRangka(det.kepala_norangka);
            setInputKepalaNoStnk(det.kepala_nostnk);
            setInputKepalaMasaBerlakuStnk(det.kepala_masaberlakustnk?moment(new Date(det.kepala_masaberlakustnk), formatdate).toDate():null);
            setInputKepalaKir(det.kepala_kir);
            setInputKepalaMasaBerlakuKir(det.kepala_masaberlakukir?moment(new Date(det.kepala_masaberlakukir), formatdate).toDate():null);
            setInputKepalaLunasTanggal(det.kepala_lunastanggal?moment(new Date(det.kepala_lunastanggal), formatdate).toDate():null);
            setInputKepalaKeterangan(det.kepala_keterangan);

            setInputBuntutNama(det.buntut_nama);
            setInputBuntutNoBuntut(det.buntut_nobuntut);
            setInputBuntutNoBearingLuar(det.buntut_nobearingluar);
            setInputBuntutNoBearingDalam(det.buntut_nobearingdalam);
            setInputBuntutNoKir(det.buntut_nokir);
            setInputBuntutMasaBerlakuKir(det.buntut_masaberlakukir?moment(new Date(det.buntut_masaberlakukir), formatdate).toDate():null);
            setSelBuntutRangkaPlat(det.buntut_rangka);
            setInputBuntutMerkAxel(det.buntut_merkaxel);
            setSelBuntutJenisAxel(det.buntut_jenisaxel);
            setSelBuntutJenisHole(det.buntut_jenishole);
            setInputBuntutLunasTanggal(det.buntut_lunastanggal?moment(new Date(det.buntut_lunastanggal), formatdate).toDate():null);

            setInputSparePartKepalaNama(det.sparepartkepala_nama);
            setSelSparePartKepalaJenisSparePart(det.sparepartkepala_jenis);
            setInputSparePartKepalaKeterangan(det.sparepartkepala_keterangan);
            setInputSparePartKepala_BearingNoBearing(det.sparepartkepala_bearing_nobearing);
            setSelSparePartKepala_BearingPosisiBearing(det.sparepartkepala_bearing_posisibearing);
            setInputSparePartKepala_BearingMerk(det.sparepartkepala_bearing_merk);
            ...
            setInputSparePartBuntut_BearingNoBearing(det.sparepartbuntut_bearing_nobearing);
            setSelSparePartBuntut_BearingPosisiBearing(det.sparepartbuntut_bearing_posisi);
            setInputSparePartBuntut_BearingMerk(det.sparepartbuntut_bearing_merk);
            ...
            setSelSparePartKepala_Filter_Posisi(det.sparepartkepala_filter_posisi);
            setSelSparePartKepala_Filter_Type(det.sparepartkepala_filter_type);
            setSelSparePartKepala_Bohlam_Type(det.sparepartkepala_bohlam_type);
            setSelSparePartKepala_Selang_Type(det.sparepartkepala_selang_type);

            setSelSparePartBuntut_Filter_Posisi(det.sparepartbuntut_filter_posisi);
            setSelSparePartBuntut_Filter_Type(det.sparepartbuntut_filter_type);
    ...
    const executeSubmit = () => {
        ...
        if(flag){
            ...
            dispatch(actions.submitEditAsset('/'+id,obj,succesHandlerSubmit, errorHandler));
        }
    }
        ...
        >
            {
                formikProps => {
                    ...
                    return(
                        <form className="mb-6" onSubmit={handleSubmit}  name="FormEditAsset">
                            <ContentWrapper>
                            <ContentHeading history={history} link={pathmenu.editAsset+'/'+id} label={'Edit Asset'} labeldefault={'Edit Asset'} />
