...
export default function EditForm(props) {
    reloadToHomeNotAuthorize(editWorkOrder_Permission,'TRANSACTION');
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getWorkOrderData('/template/'+id,successHandlerTemplate, errorHandler));
    }, []);

    const successHandlerTemplate = (data) =>{
        if(data.data){
            let val = data.data
            let template = val.templates;
            ...
            if(data.data.details){
                for(let i=0; i < data.data.details.length; i++){
                    ...
                    listitems.push({ idpartai:det.idpartai,jumlahkoli: (det.jumlahkoli !== ''? numToMoney(parseFloat(det.jumlahkoli)):""),jumlahkg:(det.jumlahkg !== ''? numToMoney(parseFloat(det.jumlahkg)):""),nocontainer:(new String(det.nocontainer).includes("NC-NODATA")?"":det.nocontainer),noseal:(new String(det.noseal).includes("NS-NODATA")?"":det.noseal),barang:det.barang});
                }
            }
            ...
            setListWoType(template.woTypeOptions.reduce((obj, el) => (
                [...obj, {
                    value: el.code,
                    label: el.codename
                }]
            ...
            setListModaTransport(template.modaTransportasiOptions.reduce((obj, el) => (
                [...obj, {
                    value: el.code,
                    label: el.codename
                }]
            ...
            setListJalur(template.jalurOptions.reduce((obj, el) => (
                [...obj, {
                    value: el.code,
                    label: el.codename
                }]
            ...
            setListPort(template.portOptions.reduce((obj, el) => (
                [...obj, {
                    value: el.id,
                    label: el.name
                }]
            ...
            let listPelayaran = template.vendorOptions.filter(output => output.vendorcategoryname == 'Shipping Line' || output.vendorcategoryname == 'Airline');
            ...
            setListPartai(template.partaiOptions.reduce((obj, el) => (
                [...obj, {
                    value: el.id,
                    label: el.name
                }]
    ...
    const executeSubmit = () => {
        ...
        if(flag){
            ...
            obj.qq = SelQQ !== '' && SelQQ !== 'nodata'?SelQQ:null;
            ...
            dispatch(actions.submitEditWorkOrder('/'+id,obj,succesHandlerSubmit, errorHandler));
        }
    }
                ...
                formikProps => {
                    ...
                    return(
                        <form className="mb-6" onSubmit={handleSubmit}  name="FormAddWorkOrder">
                            <ContentWrapper>
                            <ContentHeading history={history} link={pathmenu.editWorkOrder+'/'+id} label={'Edit Work Order'} labeldefault={'Edit Work Order'} />
