...
export default function EditFormCompany(props) {
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getCustomerData('/'+id,successHandlerDetail, errorHandler));
    ...
    function successHandlerDetail(data) {
        if(data.data){
            let cust = data.data;
            setInputName(cust.nama);
            setInputAddress(cust.address);
            setInputProvinsi(cust.provinsi);
            setInputCity(cust.city);
            setInputAreaName(cust.areaname);
            setInputSubAreaName(cust.subarename);
