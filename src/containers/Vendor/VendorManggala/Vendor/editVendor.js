...
export default function EditVendor(props) {
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getVendorData('/'+id,successHandlertemplate, errorHandler));
    }, []);
    ...
    const successHandlertemplate = (data) =>{
        if(data.data){
            ...
            setSelVendorCategory(detail.idvendorcategory);
            setSelBadanUsaha(detail.jenisbadanusaha);
    ...
    const executeSubmit = () => {
        ...
        if(flag){
            ...
            dispatch(actions.submitEditVendor('/'+id,obj,succesHandlerSubmit, errorHandler));
        }
    }
                ...
                formikProps => {
                    ...
                    return(
                        <form className="mb-6" onSubmit={handleSubmit}  name="FormAddBankAccount">
                            <ContentWrapper>
                            <ContentHeading history={history} link={pathmenu.editvendor} label={'label_EDIT_VENDOR'} labeldefault={'Edit Vendor'} />
