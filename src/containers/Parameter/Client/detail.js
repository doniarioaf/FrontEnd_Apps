...
//   import Dialog                       from '@material-ui/core/Dialog';
  ...
  import { editParameterClient_Permission,deleteParameterClient_Permission,MenuParameterClient } from '../../shared/permissionMenu';
  ...
  function Detail(props) {
    reloadToHomeNotAuthorize(MenuParameterClient,'READ');
    ...
    const handleToggle = (flag) => {
      ...
      useEffect(() => {
        setLoading(true);
        dispatch(actions.getParameterClientData('/'+id,successHandler, errorHandler));
    }, []);
    ...
    const submitHandlerDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(actions.submitDeleteParameterClient('/'+id,succesHandlerSubmit, errorHandler));
    ...
    const succesHandlerSubmit = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                history.push(pathmenu.menuParameter);
            }
        })
    }
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} link={pathmenu.detailparameter+'/'+id} label={'Detail Parameter'} labeldefault={'Detail Parameter'} />
            <Container fluid>
            <Card>
            <CardBody>
            <Button
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                    {
                        ...
                        :(<div>
                            <MenuItem hidden={!isGetPermissions(editParameterClient_Permission,'TRANSACTION')}  onClick={() => history.push(pathmenu.editparameter+'/'+id)}>{i18n.t('grid.EDIT')}</MenuItem>
                            <MenuItem hidden={!isGetPermissions(deleteParameterClient_Permission,'TRANSACTION')}  onClick={() => submitHandlerDelete()}>{i18n.t('grid.DELETE')}</MenuItem>
