...
import { numToMoneyWithComma, inputJustNumberAndCommaDot, formatMoney, reloadToHomeNotAuthorize } from '../../shared/globalFunc';
import { editParameterClient_Permission } from '../../shared/permissionMenu';
...
export default function EditParameterManggala(props) {
    reloadToHomeNotAuthorize(editParameterClient_Permission, 'TRANSACTION');
    ...
    function successHandler(data) {
        ...
        setListParamType(det.template.parameterTypeOptions.reduce((obj, el) => (
            [...obj, {
                value: el.code,
                label: el.codename
            }]
        ...
        let paramvalue = det.paramvalue;
        if (det.paramtype == 'NUMBER') {
            paramvalue = numToMoneyWithComma(det.paramvalue);
        }
        setInputvalue(paramvalue);
        setInputIsActive(det.isactive ? true : false);
        setSelParamType(det.paramtype);
        setParamDate(det.paramdate ? moment(new Date(det.paramdate), formatdate).toDate() : null);
        setLoading(false);
    }
    ...
    const executeSubmit = () => {
        ...
        if (flag) {
            ...
            dispatch(actions.submitEditParameterClient('/' + id, obj, succesHandlerSubmit, errorHandler));
        }
    }
