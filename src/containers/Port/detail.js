...
//   import Dialog                       from '@material-ui/core/Dialog';
  ...
  import { deletePort_Permission,editPort_Permission,MenuPort } from '../shared/permissionMenu';
  ...
  function Detail(props) {
    ...
    const submitHandlerDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                dispatch(actions.submitDeletePort('/'+id,succesHandlerSubmit, errorHandler));
    ...
    const succesHandlerSubmit = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                history.push(pathmenu.menuport);
            }
        })
    }
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} link={pathmenu.detailport+'/'+id} label={'Detail Port'} labeldefault={'Detail Port'} />
            <Container fluid>
            <Card>
            <CardBody>
            <Button
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                    {
                        ...
                        :(<div>
                            <MenuItem hidden={!isGetPermissions(editPort_Permission,'TRANSACTION')}  onClick={() => history.push(pathmenu.editport+'/'+id)}>{i18n.t('grid.EDIT')}</MenuItem>
                            <MenuItem hidden={!isGetPermissions(deletePort_Permission,'TRANSACTION')}  onClick={() => submitHandlerDelete()}>{i18n.t('grid.DELETE')}</MenuItem>
