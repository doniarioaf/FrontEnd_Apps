import React, {useState,
  ...
  function Detail(props) {
    ...
    const handleToggle = (flag) => {
      ...
      useEffect(() => {
        setLoading(true);
        dispatch(actions.getUserMobileData('/'+id,successHandler, errorHandler));
    }, []);
        ...
        if(data.data){
            ...
            if(data.data.callplans){
                ...
                for(let i=0; i < data.data.callplans.length ; i++){
                    let role = data.data.callplans[i];
                    arr.push(role.nama);
                }
                setCallPlans(arr.join());
            }
        }
        setLoading(false);
    }
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                    {
                        ...
                        :(<div>
                            <MenuItem hidden={false}  onClick={() => history.push(pathmenu.editusermobile+'/'+id)}>{i18n.t('grid.EDIT')}</MenuItem>
