...
export default function EditFormUserMobile(props) {
    ...
    function successHandlerDetail(data) {
        ...
        setInputNoTlp(detail.contactnumber);
        ...
        let selectedDataCallPlans = [];
        let callplans = [];
        if(data.data.callplans){
            for(let i=0; i < data.data.callplans.length ; i++){
                let cp = data.data.callplans[i];
                const options = { value: cp.id, label: cp.nama };
                selectedDataCallPlans.push(options);
                callplans.push(cp.id);
            }
        }
        setCallPlans(callplans);
        setSelectedCallPlans(selectedDataCallPlans);
    ...
    const executeSubmit = () => {
        ...
        if(flag){
            ...
            dispatch(actions.submitEditUserMobile(id,obj,succesHandlerSubmit, errorHandler));
        }
    }
