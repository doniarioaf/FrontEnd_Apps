...
export default function EditBayarKomisi(props) {
    ...
    const executeSubmit = (values) => {
        ...
        if (flag) {
            ...
            obj.items = items;
            dispatch(actions.submitKomisi({ url: '/'+id, payload: obj, type: 'EDIT' }, succesHandlerSubmit, errorHandler));
        }
    }
                ...
                formikProps => {
                    ...
                    return (
                        <form className="mb-6" onSubmit={handleSubmit} name="bayarkomisi">
                            <ContentWrapper>
                                <ContentHeading history={history} link={pathmenu.editbayarkomisi+'/'+id} label={'Edit Bayar Komisi'} labeldefault={'Edit Bayar Komisi'} />
