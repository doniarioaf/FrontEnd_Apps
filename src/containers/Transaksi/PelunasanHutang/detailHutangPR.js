...
//   import Dialog                       from '@material-ui/core/Dialog';
  ...
  function Detail(props) {
    ...
    const handleToggle = (flag) => {
      ...
      useEffect(() => {
        setLoading(true);
        dispatch(actions.getPelunasanHutangData( {url:'/detailhutangpr/'+id},successHandler, errorHandler));
    }, []);

    function successHandler(data,propsdata) {
        setValue(data.data);
        let det = data.data;
        // let detPr = det.detailPR;
        // let items = det.listpembayaran?det.listpembayaran:[];
        // let totalbayar = 0;
        // for(let i=0; i < items.length; i++){
        //     let detitems = items[i];
        //     totalbayar += detitems.amount;
        // }
        // let totalnota = detPr.totalprice - detPr.setor;
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} link={pathmenu.detailhutangpr+'/'+id} label={'Detail Hutang Purchase Receive'} labeldefault={'Detail Hutang Purchase Receive'} />
            <Container fluid>
            <Card>
            <CardBody>
            <Button
