...
const DraftPackingListIndex = () => {
    ...
    useEffect(() => {
        ...
        obj.to = to.getTime();
        dispatch(actions.getInvoiceData({ url: '/list', type: 'POST', payload: obj }, successHandler, errorHandler));
    }, []);
        ...
        if (data.data) {
            const theData = data.data.reduce((obj, el) => [
                ...obj,
                {
                    'id': el.id,
                    'nodoc': el.nodocument,
                    'nodocpl': el.nodocumentPackingList,
                    'customer': el.custNama+'/'+el.custAlias,
