...
const CancelPackingListIndex = () => {
    ...
    const [columns] = useState([
        ...
        { name: 'nodocpl', title: i18n.t('No Document PL') },
        { name: 'transdate', title: i18n.t('Date Cancel') },
    ...
    useEffect(() => {
        ...
        obj.to = to.getTime();
        dispatch(actions.getCancelPackingListData({ url: '/list', type: 'POST', payload: obj }, successHandler, errorHandler));
    }, []);
        ...
        if (data.data) {
            const theData = data.data.reduce((obj, el) => [
                ...obj,
                {
                    ...
                    'transdate': el.datecancel ? moment(el.datecancel).format(formatdate) : '',
                }
            ], []);
            setRows(theData);
        }
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} removehistorylink={true} link={pathmenu.menucancelpackinglist} label={'Cancel Packing List'} labeldefault={'Cancel Packing List'} />
            <Container fluid>
                <table>
                    <th>{'From'}</th>
...
export default CancelPackingListIndex;