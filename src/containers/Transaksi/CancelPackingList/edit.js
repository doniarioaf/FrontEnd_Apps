...
export default function CancelPackingList(props) {
    ...
    function successHandler(data, propsdata) {
        let det = data.data;
        let transDate = det.datecancel?new Date(det.datecancel):null;
        setDateTrans(transDate);
        setInputNotes(det.keterangan);
        ...
        for(let i=0; i < listItems.length > 0; i++){
            let detitems = listItems[i];
                ...
                objItems.productName = detitems.namaProduct;
                objItems.categoryProductName = detitems.categoryProductNama;
                objItems.idcategoryproduct = detitems.idcategoryproduct;
                objItems.qty = detitems.qty;
                listUdang.push(objItems);
        }
        
        setListItems(listUdang);
        setLoading(false);
    }
    ...
    const executeSubmit = (values) => {
        ...
        if (flag) {
            ...
            obj.items = items;
            dispatch(actions.submitCancelPackingList({ url: '/'+id, payload: obj, type: 'EDIT' }, succesHandlerSubmit, errorHandler));
                ...
                formikProps => {
                    ...
                    return (
                        <form className="mb-6" onSubmit={handleSubmit} name="FormCustomer">
                            <ContentWrapper>
                                <ContentHeading history={history} link={pathmenu.cancelpackinglist+'/'+id} label={'Edit Cancel Packing List'} labeldefault={'Edit Cancel Packing List'} />
