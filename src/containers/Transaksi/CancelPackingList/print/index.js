...
import { MenuCancelPackingList } from '../../../shared/permissionMenu';
...
export default function PrintCancelPackingList(props) {
    reloadToHomeNotAuthorize(MenuCancelPackingList, 'READ');
    ...
    const fileName = "CancelPackingList";

    useEffect(() => {
        setLoading(true);
        dispatch(actions.getCancelPackingListData( {url:'/print/'+id},successHandler, errorHandler));
    }, [dispatch]);
    ...
    const handleSuccesPDF = (dataUrl, namaFile) => {
        ...
        fileLink.download = 'CancelPackingList-'+moment(new Date()).format(formatdateYYYYMMDD)+'-'+namaFile + '.pdf';
        fileLink.click();
        fileLink.remove();
    ...
    // const handleDownloadPDF = () => {
        ...
        dispatch(actions.getCancelPackingListData({ url: '/catatdownload/' + id }, successHandlerDownload, errorHandler));
    }
    function successHandlerDownload(data, propsdata){
        dispatch(actions.getCancelPackingListData( {url:'/print/'+id},successHandlerAfterDownload, errorHandler));
    }
    ...
    return (
            <div>
                <ContentWrapper>
                    <ContentHeading history={history} link={pathmenu.printpdfcancelpackinglist + '/' + id} label={'PDF Cancel Packing List'} labeldefault={'PDF Cancel Packing List'} />
                    <Container fluid>
                        <Card>
                            <CardBody>
