...
export default function AddCargo(props) {
    reloadToHomeNotAuthorize(addCargo_Permission, 'TRANSACTION');
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getCargoData({ url: '/template' }, successHandler, errorHandler));
    }, []);
    ...
    const executeSubmit = (values) => {
        ...
        if (flag) {
            ...
            dispatch(actions.submitCargo({ url: '', payload: obj, type: 'ADD' }, succesHandlerSubmitData, errorHandler));
        }
    }
                ...
                formikProps => {
                    ...
                    return (
                        <form className="mb-6" onSubmit={handleSubmit} name="FormVendor">
                            <ContentWrapper>
                                <ContentHeading history={history} link={pathmenu.addcargo} label={'Add Cargo'} labeldefault={'Add Cargo'} />
