...
import { MenuReportKartuDeposit } from '../shared/permissionMenu';
import { formatdate } from '../shared/constantValue';
import * as pathmenu           from '../shared/pathMenu';
import "react-widgets/dist/css/react-widgets.css";

export default function ReportKartuDeposit(props) {
    reloadToHomeNotAuthorize(MenuReportKartuDeposit,'READ');
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getReport({ url: '/reportkartudeposit/template' }, successHandler, errorHandler));
    }, []);
    ...
    const submitHandler = () => {
        if( start != null && end != null && SelVendor.length > 0){
            ...
            setLoading(true);
            dispatch(actions.getReport({ url: '/reportkartudeposit?from=' + start.getTime() + '&to=' + end.getTime()+'&idvendors='+idvendor+'&shownol='+SelShowNol,type:'GETFILE',typefile:'application/vnd.ms-excel' }, successHandlerReport, errorHandler));
    ...
    const successHandlerReport = (data) => {
        ...
        fileLink.download = 'ReportKartuDeposit.xlsx';
        fileLink.click();
        fileLink.remove();
        setLoading(false);
                ...
                formikProps => {
                    ...
                    return(
                        <form className="mb-6" onSubmit={handleSubmit}  name="formReportStatusInvoice">
                            <ContentWrapper>
                            <ContentHeading history={history} removehistorylink={true} link={pathmenu.menuReportKartuDeposit} label={'Laporan Kartu Deposit'} labeldefault={'Laporan Kartu Deposit'} />
