...
import { MenuReportInvoice } from '../../shared/permissionMenu';
...
export default function ReportInvoice(props) {
    reloadToHomeNotAuthorize(MenuReportInvoice,'READ');
    ...
    const [ListShowAll, setListShowAll] = useState([{value:'LUNAS',label:'Lunas'},{value:'BELUMLUNAS',label:'Belum Lunas'},{value:'ALL',label:'All'}]);
    const [SelShowAll, setSelShowAll] = useState('ALL');
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getReportData('/manggala/reportinvoice/template','',succesHandlerSubmitTemplate, errorHandler));
    }, []);
    const succesHandlerSubmitTemplate = (data) =>{
        if(data.data){
            let listCustomer = [];
            listCustomer = data.data.customerOpt.reduce((obj, el) => (
                [...obj, {
                    value: el.id,
            ...
            listCustomer.push({value:'ALL',label:'All'});
    ...
    const submitHandler = () => {
        if(start != null && end != null){
            ...
            let pathURL = '/manggala/reportinvoice?from='+startDate+'&to='+thruDate+'&type='+typereport+'&listcustomerid='+SelCustomer+'&showall='+SelShowAll;
            if(output == 'XLSX'){
    ...
    const succesHandlerSubmit = (data) => {
        ...
        fileLink.download = 'ReportInvoice.xlsx';
        fileLink.click();
        fileLink.remove();
        setLoading(false);
    ...
    const handleChangeShowALL = (data) =>{
        let id = data?.value ? data.value : '';
        setSelShowAll(id);
    }
                ...
                formikProps => {
                    ...
                    return(
                        <form className="mb-6" onSubmit={handleSubmit}  name="formReportSummaryKegiatanTruck">
                            <ContentWrapper>
                            <ContentHeading history={history} removehistorylink={true} link={pathmenu.reportInvoice} label={'Report Invoice'} labeldefault={'Report Invoice'} />
                            ...
                            <div className="mt-2 col-lg-6 ft-detail mb-5">
                            <label className="mt-3 form-label required" htmlFor="idcustomer">
                                {i18n.t('Customer')}
                            </label>
                            ...
                            <DropdownList
                                    ...
                                    onBlur={val => setFieldTouched("idcustomer", val?.value ? val.value : '')}
                                    data={ListCustomer}
                                    textField={'label'}
                                    valueField={'value'}
                                    // style={{width: '25%'}}
                            ...
                            <label className="mt-3 form-label required" htmlFor="showall">
                                {i18n.t('Show All?')}
                            </label>
                            ...
                            <DropdownList
                                    ...
                                    onChange={val => handleChangeShowALL(val)}
                                    onBlur={val => setFieldTouched("showall", val?.value ? val.value : '')}
                                    data={ListShowAll}
                                    textField={'label'}
                                    valueField={'value'}
                                    // style={{width: '25%'}}
