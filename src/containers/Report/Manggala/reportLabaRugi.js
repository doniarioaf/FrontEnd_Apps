...
export default function ReportKasBank(props) {
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getReportData('/manggala/labarugi/template','',succesHandlerSubmitTemplate, errorHandler));
    }, []);
    const succesHandlerSubmitTemplate = (data) =>{
        let listBank = [];
        
        if(data.data){
            listBank = data.data.reduce((obj, el) => (
                [...obj, {
                    value: el.id,
                    label: el.namabank
                }]
    ...
    const submitHandler = () => {
        if(start != null && end != null && SelBank !== ''){
            let idbank = SelBank == 'ALL'?0:SelBank;
            setLoading(true);
            ...
            let pathURL = '/manggala/labarugi?from='+startDate+'&thru='+thruDate+'&type='+typereport+'&idbank='+idbank;
            if(output == 'XLSX'){
    ...
    const succesHandlerSubmit = (data) => {
        ...
        fileLink.download = 'ReportLabaRugi.xlsx';
        fileLink.click();
        fileLink.remove();
        setLoading(false);
