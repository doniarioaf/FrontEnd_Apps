...
import { MenuReportPelunasanPiutang } from '../shared/permissionMenu';
import { formatdate } from '../shared/constantValue';
import * as pathmenu           from '../shared/pathMenu';
import "react-widgets/dist/css/react-widgets.css";

export default function ReportPelunasanPiutang(props) {
    reloadToHomeNotAuthorize(MenuReportPelunasanPiutang,'READ');
    ...
    useEffect(() => {
        setLoading(true);
        dispatch(actions.getReport({ url: '/reportpelunasanpiutang/template' }, successHandler, errorHandler));
    }, []);
    ...
    const submitHandler = () => {
        ...
        if( start != null && end != null && flag){
            ...
            setLoading(true);
            dispatch(actions.getReport({ url: '/reportpelunasanpiutang?from=' + start.getTime() + '&to=' + end.getTime()+'&grups='+customergrup+'&idcustomer='+idcustomer+'&status='+SelStatus,type:'GETFILE',typefile:'application/vnd.ms-excel' }, successHandlerReport, errorHandler));
    ...
    const successHandlerReport = (data) => {
        ...
        fileLink.download = 'ReportPelunasanPiutang.xlsx';
        fileLink.click();
        fileLink.remove();
        setLoading(false);
                ...
                formikProps => {
                    ...
                    return(
                        <form className="mb-6" onSubmit={handleSubmit}  name="formReportStatusInvoice">
                            <ContentWrapper>
                            <ContentHeading history={history} removehistorylink={true} link={pathmenu.menuReportPelunasanPiutang} label={'Laporan Pelunasan Piutang'} labeldefault={'Laporan Pelunasan Piutang'} />
