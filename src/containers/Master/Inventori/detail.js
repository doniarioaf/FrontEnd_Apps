...
//   import Dialog                       from '@material-ui/core/Dialog';
  ...
  import { editInventori_Permission,deleteInventori_Permission,MenuInventori } from '../../shared/permissionMenu';
  ...
  function Detail(props) {
    ...
    const handleToggle = (flag) => {
      ...
      useEffect(() => {
        setLoading(true);
        dispatch(actions.getInventoriData( {url:'/'+id},successHandler, errorHandler));
    }, []);
    ...
    const submitHandlerDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                setLoading(true);
                dispatch(actions.submitInventoriData( {url:'/'+id,type:'DELETE'} ,succesHandlerSubmit, errorHandler));
    ...
    const succesHandlerSubmit = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                history.push(pathmenu.menuInventori);
            }
        })
    }
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} link={pathmenu.detailInventori+'/'+id} label={'Detail'} labeldefault={'Detail'} />
            <Container fluid>
            <Card>
            <CardBody>
            <Button
            ...
            <CardBody>
                {
                    ...
                    (
                        <section>
                            ...
                            <div className="row mt-3">
                            <span className="col-md-5">{i18n.t('SKU')}</span>
                            <strong className="col-md-7">
                                {value.sku?value.sku:''}
                            </strong>
                            </div>
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                    {
                        ...
                        :(<div>
                            <MenuItem hidden={!isGetPermissions(editInventori_Permission,'TRANSACTION')}  onClick={() => history.push(pathmenu.editInventori+'/'+id)}>{i18n.t('grid.EDIT')}</MenuItem>
                            <MenuItem hidden={!isGetPermissions(deleteInventori_Permission,'TRANSACTION')}  onClick={() => submitHandlerDelete()}>{i18n.t('grid.DELETE')}</MenuItem>
