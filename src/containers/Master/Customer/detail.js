...
//   import Dialog                       from '@material-ui/core/Dialog';
  ...
  import { editCustomer_Permission,deleteCustomer_Permission,MenuCustomer } from '../../shared/permissionMenu';
  ...
  function Detail(props) {
    ...
    const submitHandlerDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                setLoading(true);
                dispatch(actions.submitCustomerData( {url:'/'+id,type:'DELETE'} ,succesHandlerSubmit, errorHandler));
    ...
    const succesHandlerSubmit = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                history.push(pathmenu.menuCustomer);
            }
        })
    }
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} link={pathmenu.detailCustomer+'/'+id} label={'Detail'} labeldefault={'Detail'} />
            <Container fluid>
            <Card>
            <CardBody>
            <Button
                    ...
                    (
                        <section>
                            ...
                            <span className="col-md-5">{i18n.t('label_ACC_NO')}</span>
                                <strong className="col-md-7">
                                {value.banknumber ?value.banknumber:''}
                                </strong>
                            </div>
                            ...
                            <span className="col-md-5">{i18n.t('label_ACC_NAME')}</span>
                                <strong className="col-md-7">
                                {value.accountbankname ?value.accountbankname:''}
                                </strong>
                            </div>
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                    {
                        ...
                        :(<div>
                            <MenuItem hidden={!isGetPermissions(editCustomer_Permission,'TRANSACTION')}  onClick={() => history.push(pathmenu.editCustomer+'/'+id)}>{i18n.t('grid.EDIT')}</MenuItem>
