...
//   import Dialog                       from '@material-ui/core/Dialog';
  ...
  import { editCategoryProduct_Permission,deleteCategoryProduct_Permission,MenuCategoryProduct } from '../../shared/permissionMenu';
  ...
  function Detail(props) {
    ...
    const handleToggle = (flag) => {
      ...
      useEffect(() => {
        setLoading(true);
        dispatch(actions.getCategoryProductData( {url:'/'+id},successHandler, errorHandler));
    }, []);
    ...
    const submitHandlerDelete = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                setLoading(true);
                dispatch(actions.submitCategoryProductData( {url:'/'+id,type:'DELETE'} ,succesHandlerSubmit, errorHandler));
    ...
    const succesHandlerSubmit = (data) => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                history.push(pathmenu.menuCategoryProduct);
            }
        })
    }
    ...
    return (
        <ContentWrapper>
            <ContentHeading history={history} link={pathmenu.detailcategoryproduct+'/'+id} label={'Detail'} labeldefault={'Detail'} />
            <Container fluid>
            <Card>
            <CardBody>
            <Button
            ...
            <CardBody>
                {
                    ...
                    (
                        <section>
                            ...
                            <div className="row mt-3">
                            <span className="col-md-5">{i18n.t('Size')}</span>
                            <strong className="col-md-7">
                ...
                <MenuList autoFocusItem={open} id="menu-list-grow" onKeyDown={handleListKeyDown}>
                    {
                        ...
                        :(<div>
                            <MenuItem hidden={!isGetPermissions(editCategoryProduct_Permission,'TRANSACTION')}  onClick={() => history.push(pathmenu.editcategoryproduct+'/'+id)}>{i18n.t('grid.EDIT')}</MenuItem>
                            <MenuItem hidden={!isGetPermissions(deleteCategoryProduct_Permission,'TRANSACTION')}  onClick={() => submitHandlerDelete()}>{i18n.t('grid.DELETE')}</MenuItem>
