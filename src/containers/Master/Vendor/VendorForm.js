...
/**
 * Reusable Vendor form, shared by AddVendor and EditVendor pages.
 *
 * Props:
 * - mode: 'ADD' | 'EDIT'
 * - id: vendor id (only relevant when mode === 'EDIT')
 * - headingLink: link passed to <ContentHeading />
 * - headingLabel: label passed to <ContentHeading />
 */
export default function VendorForm({ mode, id, headingLink, headingLabel }) {
    ...
    const [ListYesNo, setListYesNo] = useState([{ value: 'Y', label: 'Yes' }, { value: 'N', label: 'No' }]);
    const [SelCanLoan, setSelCanLoan] = useState("N");
    const [SelCanDeposit, setSelCanDeposit] = useState("N");

    useEffect(() => {
        setLoading(true);
        if (isEdit) {
            dispatch(actions.getVendorData({ url: '/' + id }, successHandlerVendor, errorHandler));
        } else {
            dispatch(actions.getVendorData({ url: '/template' }, successHandlerTemplateOnly, errorHandler));
        }
    }, []);

    // ADD mode: template only, no existing vendor values to populate.
    function successHandlerTemplateOnly(data, propsdata) {
        if (data.data) {
            applyTemplateLists(data.data);
        }
        setLoading(false);
    }

    // EDIT mode step 1: got vendor data, now fetch the template, carrying the vendor data along.
    function successHandlerVendor(data, propsdata) {
        let val = data.data;
        dispatch(actions.getVendorData({ url: '/template', propsdata: val }, successHandlerVendorTemplate, errorHandler));
    }

    // EDIT mode step 2: got template, propsdata is the vendor data from step 1.
    function successHandlerVendorTemplate(data, propsdata) {
        let val = propsdata;
        ...
        setSelCanLoan(val.canloan ? 'Y' : 'N');
        setSelCanDeposit(val.candeposit ? 'Y' : 'N');
        ...
        applyTemplateLists({ ...template, vendorBrokerOpt: data.data.vendorBrokerOpt, areaOpt: data.data.areaOpt });

        setLoading(false);
    }

    // Shared: turns template payload into the dropdown option lists used by both Add and Edit.
    function applyTemplateLists(template) {
        ...
        if (template.vendorParentOpt) {
            const theDataVend = template.vendorParentOpt.reduce((obj, el) => [
                ...obj,
                {
                    value: el.id,
                    label: el.nama,
        ...
        if (template.vendorBrokerOpt) {
            theDataVendBroker = template.vendorBrokerOpt.reduce((obj, el) => [
                ...obj,
                {
                    value: el.id,
                    label: el.nama,
        ...
        if (template.areaOpt) {
            let dataArea = template.areaOpt.reduce((obj, el) => [
                ...obj,
                {
                    value: el.id,
                    label: el.nama,
    ...
    const executeSubmit = (values) => {
        ...
        if (flag) {
            ...
            if(CheckIsParent){
                //khusus subparent,jika parent set N saja
                obj.canloan = 'N';
                obj.candeposit = 'N';
            }else{
                obj.canloan = SelCanLoan;
                obj.candeposit = SelCanDeposit;
            }
            ...
            dispatch(actions.submitVendorData({ url, payload: obj, type }, succesHandlerSubmit, errorHandler));
        }
    }
    ...
    const handleChangeCanLoan = (data) => {
        let id = data?.value ? data.value : '';
        setSelCanLoan(id);
    }

    const handleChangeCanDeposit = (data) => {
        let id = data?.value ? data.value : '';
        setSelCanDeposit(id);
    }
                ...
                formikProps => {
                    ...
                    return (
                        <form className="mb-6" onSubmit={handleSubmit} name="FormVendor">
                            <ContentWrapper>
                                <ContentHeading history={history} link={headingLink} label={headingLabel} labeldefault={headingLabel} />
                                ...
                                <div className="row mt-2">
                                    <div className="mt-2 col-lg-6 ft-detail mb-5">
                                        ...
                                        <div hidden={values.isparent}>
                                            <label className="mt-3 form-label required" htmlFor="CanLoan">
                                                {i18n.t('Izinkan Pinjaman?')}
                                            </label>
                                            <DropdownList
                                                name="CanLoan"
                                                filter='contains'
                                                placeholder={i18n.t('select.SELECT_OPTION')}
                                                onChange={val => handleChangeCanLoan(val)}
                                                onBlur={val => setFieldTouched("CanLoan", val?.value ? val.value : '')}
                                                data={ListYesNo}
                                                textField={'label'}
                                                valueField={'value'}
                                                value={values.CanLoan}
                                            />

                                            <label className="mt-3 form-label required" htmlFor="CanDeposit">
                                                {i18n.t('Izinkan Deposit?')}
                                            </label>
                                            <DropdownList
                                                name="CanDeposit"
                                                filter='contains'
                                                placeholder={i18n.t('select.SELECT_OPTION')}
                                                onChange={val => handleChangeCanDeposit(val)}
                                                onBlur={val => setFieldTouched("CanDeposit", val?.value ? val.value : '')}
                                                data={ListYesNo}
                                                textField={'label'}
                                                valueField={'value'}
                                                value={values.CanDeposit}
                                            />
                                        </div>
